...
{
  read_xml_to_dictionary(data_file_path_);
    ...
    const date_struct &today_date, unsigned int &study_count)
{
  ...
  if (0 == word_map.size()) {
    std::cout << "nothing data." << std::endl;
    getch();
    return;
  }
...
void y0_word_card::word_card::do_function(const int &kselect_num)
{
  switch (kselect_num) {
    case STUDY:
      study_processing();
      write_dictionary_to_xml(data_file_path_);
      std::cout << "finish" << '\n';
      break;
